...
class FloodControllerTest {
	...
	void testGetHouseholdsByStations_ReturnsResults() throws Exception {
		...
		when(floodService.getHouseholdsByStations(List.of(1, 2))).thenReturn(List.of(household1, household2));
	...
	void testGetHouseholdsByStations_EmptyResult() throws Exception {
		when(floodService.getHouseholdsByStations(List.of(99))).thenReturn(List.of());

		mockMvc.perform(get("/flood/stations").param("stations", "99")).andExpect(status().isOk())
