...
class CommunityEmailControllerTest {
	...
	void getEmails_shouldReturnEmailList() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/communityEmail").param("city", "Bordeaux"))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0]").value("test1@mail.com"))
	...
	void getEmails_shouldReturn404_whenNoEmails() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/communityEmail").param("city", "Inconnue"))
