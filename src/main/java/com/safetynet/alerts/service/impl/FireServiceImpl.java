...
/**
 * Implémentation du service FireService qui fournit les informations liées à
 * une adresse spécifique.
 *
 * Cette classe récupère : - les personnes vivant à une adresse donnée, - le
 * numéro de la caserne de pompiers qui couvre cette adresse, - les dossiers
 * médicaux des personnes, - et calcule leur âge pour constituer une réponse
 * structurée.
 *
 * Elle est utilisée par le contrôleur FireController pour répondre à l’endpoint
 * REST GET /fire.
 */
@Service
public class FireServiceImpl implements FireService {
	...
	public FireServiceImpl(PersonRepository personRepository, MedicalRecordRepository medicalRecordRepository,
	...
	/**
	 * Récupère les informations complètes pour une adresse : - les personnes qui y
	 * vivent, - le numéro de la caserne qui couvre cette adresse, - leur téléphone,
	 * âge, médicaments et allergies.
	 *
	 * @param address l’adresse à interroger
	 * @return un objet FireResponseDTO contenant les résidents et la caserne
	 *         associée
	 */
	@Override
	public FireResponseDTO getFireInfoByAddress(String address) {
		if (logger.isDebugEnabled()) {
			logger.debug("Requête reçue pour l'adresse : {}", address);
		}
		...
		List<FirePersonDTO> personDTOs = persons.stream().map(person -> {
			...
			return new FirePersonDTO(person.getFirstName(), person.getLastName(), person.getPhone(), age,
